...
def main():
    ...
    print(
        f"reached AMBER: {len(amber) / len(fake_rows):.1%}   reached RED: {len(det) / len(fake_rows):.1%}"
    )
    if amber:
    ...
    within5 = np.mean(
        [r["first_amber_t"] is not None and r["first_amber_t"] <= 5.0 for r in fake_rows]
    )
    within10 = np.mean(
        [r["first_red_t"] is not None and r["first_red_t"] <= 10.0 for r in fake_rows]
    )
    ...
    w10_long = np.mean(
        [r["first_red_t"] is not None and r["first_red_t"] <= 10.0 for r in long_fakes]
    )

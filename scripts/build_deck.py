...
def footer(s, n):
    text(
        ...
        Inches(6),
        Inches(0.3),
        "VoiceShield · Team Jigyasa",
        size=9,
        color=GREY,
    )
    text(
        s,
        SLIDE_W - MARGIN - Inches(0.5),
        Inches(7.05),
        Inches(0.5),
        Inches(0.3),
        str(n),
...
for i, (big, small) in enumerate(stats):
    ...
    text(s, x + Inches(0.3), Inches(3.4), cw - Inches(0.6), Inches(0.9), small, size=14, color=GREY)
text(
    s,
    MARGIN,
    Inches(5.0),
...
bullets(
    ...
    [
        (
            "Advisory, not judge.",
            "The agent stays in control; VoiceShield names the evidence behind every alert.",
        ),
        (
            "Privacy by construction.",
            "A 10-second in-memory rolling buffer — no call recording ever touches disk.",
        ),
        (
            "Explainable.",
            "Each alert cites the artifact: phase discontinuity, over-smooth pitch, vocoder band energy.",
        ),
    ],
    size=16,
)
...
t = text(
    ...
    Inches(3.6),
    Inches(0.35),
    "suspicion ≥ threshold — or periodic probe",
    size=11,
    color=GREY,
    align=PP_ALIGN.CENTER,
)
elbow = s.shapes.add_shape(
    MSO_SHAPE.DOWN_ARROW, MARGIN + Inches(8.6), y1 + bh + Inches(0.42), Inches(0.32), Inches(0.44)
)
elbow.fill.solid()
elbow.fill.fore_color.rgb = LINE
elbow.line.fill.background()
...
p2 = tf.add_paragraph()
p2.text = (
    "XLS-R 300M · WavLM · phase/pitch rules — three input representations, two training domains"
)
p2.alignment = PP_ALIGN.CENTER
for r in p2.runs:
    r.font.name = FONT
    r.font.size = Pt(12)
    r.font.color.rgb = RGBColor(0xBF, 0xBF, 0xBF)
...
rows = [
    (
        "MMS-300M (NII)",
        "multilingual SSL, 74k h + RawBoost",
        "all families: GAN vocoders, TTS, VC, wild fakes",
        "benchmark AUC 0.997",
    ),
    (
        "XLS-R 300M",
        "self-supervised speech model",
        "vocoder fingerprints in embeddings",
        "benchmark AUC 0.73",
    ),
    (
        "WavLM-base",
        "self-supervised, In-the-Wild",
        "modern commercial TTS (ElevenLabs-class)",
        "benchmark AUC 0.87",
    ),
    (
        "Phase / Pitch",
        "DSP rules — fully explainable",
        "artifact naming for the agent, zero fusion weight",
...
cards = [
    (
        ...
        "Every AMBER/RED names its artifact; validated on Hindi as well as English "
        "(EER 0%). Agents act on reasons, not black-box scores.",
    ),
    (
        "Privacy by design",
        "10-second in-memory buffer, zero persistent recording. Evidence saved only on "
        "explicit save-for-audit — aligned with Indian data-privacy expectations.",
    ),
    (
        "Same voice, still human",
        "Beyond deepfake detection, ECAPA speaker-tracking flags a mid-call takeover "
        "or splice — the human-fraudster case biometrics miss.",
    ),
    (
        "Court-ready evidence",
        "One click exports spectrogram + phase heatmap, SHA-256 audio hash, and the full "
...
rows = [
    ...
    (
        "Genuine speech — 173 clips incl. codec + noisy wild domains",
        "3.5% false-RED · demo: zero",
        S_GREEN,
    ),
...
bullets(
    ...
    [
        (
            "Evidence-calibrated fusion.",
            "We benchmarked 8 detectors on 496 clips and encoded each model's measured "
        ...
        (
            "Honest metrics.",
            "We report EER (3.1%) and precision at a realistic 1% fraud rate, not just a flattering "
            "AUC. That analysis is why the pipeline uses 2-chunk hysteresis — it lifts alert precision from 18% to 82%.",
        ),
        (
            "Robustness engineered, not assumed.",
            "Neural VAD holds in noise where energy gating calls pure noise "
...
conn = s.shapes.add_shape(
    MSO_SHAPE.RIGHT_ARROW, MARGIN + Inches(9.28), y + Inches(0.58), Inches(0.24), Inches(0.16)
)
conn.fill.solid()
...
bullets(
    ...
    [
        (
            "Integration surface is two URLs.",
            "Versioned REST + WebSocket (/v1 frozen, /v2 ensemble) — CRM and "
            "agent-assist tools consume JSON; no telephony rework.",
        ),
        (
            "Compliance posture.",
            "No cloud dependency, no stored voice data, inference entirely on-premise; evidence "
            "export is explicit, hashed, and auditable.",
        ),
        (
            "Sizing.",
            "166 ms per 500 ms chunk on a laptop GPU today; one datacenter GPU serves multiple concurrent "
...
steps = [
    (
        "1",
        "Genuine voice — live microphone",
        "Speak into the browser; verdict stays GREEN, artifacts panel stays quiet.",
    ),
    (
        "2",
        "Real ElevenLabs voice clone",
        "Upload the cloned clip: AMBER at 3 s, RED at 5 s — watch the per-model score bars disagree, and fusion decide.",
    ),
    (
        "3",
        "Evidence export",
        "One click produces the audit package: spectrogram, phase heatmap, SHA-256, score envelope.",
    ),
]
y = Inches(2.2)
for n, t, d in steps:
    ...
    text(
        s, MARGIN + Inches(0.75), y - Inches(0.02), Inches(10.5), Inches(0.4), t, size=18, bold=True
    )
    text(
        s,
        MARGIN + Inches(0.75),
...
bullets(
    ...
    [
        (
            ...
            "Live browser-mic capture → GPU ensemble → advisory dashboard, running "
            "end-to-end today at p95 166 ms per chunk.",
        ),
        (
            "Engineering rigor.",
            "94 automated tests including acceptance gates (false-alert rate, detection-time, "
            "latency); CI-gated; structured audit logging with trace IDs.",
        ),
        (
            "Validated against real attacks.",
            "Detected a genuine ElevenLabs clone of a real voice — the exact attack "
...
phases = [
    ...
    (
        "+6 months",
        "India-ready",
        "Hindi + regional-language robustness ·\nmultilingual anti-spoof model · agent rollout",
        False,
    ),
    (
        "+12 months",
        "Network effect",
        "cross-branch fraud analytics · voice-clone\nthreat intel shared across PSBs",
        False,
    ),
]
...
for i, (when, t, d, now) in enumerate(phases):
    ...
    dot.fill.solid()
    dot.fill.fore_color.rgb = ACCENT if now else INK
